...
class ProductTableApp {
  ...
  defineElements($el, products) {
    ...
    this.$filterBrand = this.$el
      .find('#filter-brand')
      .append(brands.map((brand) => `<option value="${brand}">${brand}</option>`))
    this.$filterCategory = this.$el
      .find('#filter-category')
      .append(categories.map((category) => `<option value="${category}">${category}</option>`))
  ...
  filter(products) {
    ...
    const isBrandValid = (product) => {
      return $selectedBrand.val() === 'all' ? product : product.brand === $selectedBrand.text()
    }
